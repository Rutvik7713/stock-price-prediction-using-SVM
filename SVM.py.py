...
df=pd.read_csv('/content/GSFC.NS.csv')
...
def predict_prices(dates, prices, x):   
    ...
    plt.scatter(dates,svr_rbf.predict(dates),color='yellow',label='RBF model')
    plt.scatter(dates,svr_lin.predict(dates),color='green',label='Linear Model')
    plt.scatter(dates,svr_rbf.predict(dates),color='red',label='Polynomial model')
    plt.xlabel('Date')
    plt.ylabel('price')
    plt.title('Support vactor Regression')
    plt.legend()
    plt.show()
